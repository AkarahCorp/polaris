...
public class AchievementLogic {
    public static void activate(ServerPlayer player, Identifier location) {
        for(var ach : Resources.ACHIEVEMENT_KIND.registry().entrySet()) {
            if(ach.getValue().triggers().contains(location) && ach.getValue().criteria().evaluate(player)) {
                try {
                    Resources.actionManager().performEvents("player.complete_achievement", REntity.of(player), RIdentifier.of(ach.getKey().identifier()));
                } catch (Throwable e) {
